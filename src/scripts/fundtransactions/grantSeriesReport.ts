...
async function createGrantSeriesReport() {
    ...
    const headers = {
        seriesTransactionCode: 'Grant Series Code',
        fund: 'Fund',
        amount: 'Amount',
        recipient: 'Recipient',
    ...
    for (const grant of grantSeries) {
        const instances = await fundTransactionRepo
            .createQueryBuilder('fundTransaction')
            .innerJoinAndSelect('fundTransaction.transactionStatus', 'transactionStatus')
            .innerJoinAndSelect('fundTransaction.transactionType', 'transactionType')
            .where('fundTransaction.originalFundTransactionId = :seriesId', { seriesId: grant.id })
            .andWhere('transactionType.name = :grantType', {
                grantType: TransactionTypeValue.GRANT
            })
            .orderBy('fundTransaction.scheduledDate', 'ASC')
            .getMany();
        const recurrenceRule = grant.transactionRecurrence.recurrenceRule;
        ...
        const paymentType = grant.metadata && grant.metadata.paymentDetails ?
        ...
        for (let i = 0; i < (difference > 0 ? numberOfExpectedInstances : numberOfInstances); i++) {
            const row = {} as Row;
            row['seriesTransactionCode'] = grant.transactionCode;
            row['fund'] = grant.fund.fundKey;
            row['amount'] = grant.amount.toString();
            row['recipient'] = grant.transactionInfo.recipient.name;
    ...
    const file = path.resolve(__dirname, 'grantSeriesReport.csv');
