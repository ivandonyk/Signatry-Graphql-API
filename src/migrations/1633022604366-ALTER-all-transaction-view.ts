...
export class ALTERAllTransactionView1633022604366 implements MigrationInterface {
    private functionName =
        'get_next_recurrence_schedule_date(start_date date, end_date date, frequency character varying, the_interval integer, num_grants integer)';
    private viewName = 'vw_recurring_records_to_process';

    private dropEntities = async (queryRunner: QueryRunner) => {
        await queryRunner.query(`DROP VIEW ${this.viewName};`);
        await queryRunner.query(`DROP FUNCTION ${this.functionName};`);
    };

    public async up(queryRunner: QueryRunner): Promise<void> {
        await this.dropEntities(queryRunner);
        /**
         * updates:
         * if num_grants === 0 then use then assign `start_date` to `scheduled_date`
         */
        ...
        /**
         * updates:
         * add series transaction_code for debugging
         * filter out `is_historic` series
         * remove fund_transaction -> original_fund_transaction join
         * refactor transaction_type to use fund_transaction
         * check for series types
         *
         * for "r.id NOT IN () section":
         * filter for grant/contributions (not series)
         * remove union since info.requestedProcessDate isn't used any more
         */
        await queryRunner.query(`
            ...
            FROM (
                ...
                FROM ( SELECT DISTINCT
                    ...
                    WHERE
                        ...
                        AND (tt.name::text = ANY (ARRAY['GRANT_SERIES'::character varying, 'CONTRIBUTION_SERIES'::character varying]::text[]))
                        AND NOT (r.id IN (
                                SELECT
                                    outer_recurrence.id
                                FROM (
                                ...
                                WHERE ((outer_recurrence.id, date_trunc('day'::text, outer_recurrence.next_scheduled_date::timestamp with time zone)) IN (
                                    ...
                                    WHERE
                                        tt_1.name::text = ANY (ARRAY['CONTRIBUTION'::character varying, 'GRANT'::character varying]::text[])))))) inner_view_data) view_data
            WHERE ((
                    SELECT
                        view_data.next_scheduled_date
                    FROM
