...
export class addSourceDestinationAccountsAllTransactionView1638467176495
    implements MigrationInterface {
    private view = 'vw_all_transaction';

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`DROP MATERIALIZED VIEW ${this.view};`);
        ...
        all_trans.fund_code,
        all_trans.fund_key,
        all_trans.source_account_title,
        all_trans.source_account_number,
        all_trans.source_account_id,
        all_trans.destination_account_title,
        all_trans.destination_account_number,
        all_trans.destination_account_id,
       ...
       FROM ( SELECT ft.id,
            ...
            UNION ALL
             SELECT ftd.id,
                ...
                gas.title AS source_account_title,
                gas.account_number AS source_account_number,
                ftd.source_glaccount_id AS source_account_id,
			    gad.title AS destination_account_title,
                gad.account_number AS destination_account_number,
                ftd.destination_glaccount_id AS destination_account_id
               FROM fund_transaction_detail ftd
                 ...
                 JOIN gl_account gas ON ftd.source_glaccount_id = gas.id
			    JOIN gl_account gad ON ftd.destination_glaccount_id = gad.id
              WHERE tdt.name::text = ANY (ARRAY['INVESTMENT'::character varying::text, 'DIVESTMENT'::character varying::text, 'TRANSFER'::character varying::text, 'CASH_IN'::character varying::text, 'CASH_OUT'::character varying::text])) all_trans;`);
    }
