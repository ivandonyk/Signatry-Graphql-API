...
export class ALTERUpdatePermissionAddAdminTransfer1613496811850 implements MigrationInterface {
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(/* sql */ `
            INSERT INTO permission
                (name, description, access_level, access_type, role_id)
            SELECT 'Admin Fund Transfers', 'Admin Fund Transfers', 'READ', 'ADMIN_FUND_TRANSFERS', id
                FROM role;
        `);
        await queryRunner.query(/* sql */ `
            UPDATE permission SET access_level = 'NONE' WHERE role_id = (SELECT id FROM role WHERE name = 'Donor');
        `);
        await queryRunner.query(/* sql */ `
            UPDATE permission SET access_level = 'FULL' WHERE role_id in (SELECT id FROM role WHERE name in ('STAFF_PLUS', 'STAFF_FINANCE_EXECUTIVE', 'STAFF_ADMIN', 'GLOBAL_ADMIN', '_MASTER'));
        `);
        await queryRunner.query(/* sql */ `
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(/* sql */ `
            DELETE FROM permission WHERE access_type = 'ADMIN_FUND_TRANSFERS';
        `);
        await queryRunner.query(/* sql */ `
            ALTER TYPE permission_access_type RENAME TO "_permission_access_type";
        `);
