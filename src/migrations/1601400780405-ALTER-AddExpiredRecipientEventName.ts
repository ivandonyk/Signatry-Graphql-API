...
export class ALTERAddExpiredRecipientEventName1601400780405 implements MigrationInterface {
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(/*sql*/ `
                -- 1. rename the enum type you want to change
                ALTER TYPE recipient_event_name rename to _recipient_event_name;
                -- 2. create new type
                CREATE TYPE recipient_event_name as ENUM ('EXPIRED','PREFERRED PAYMENT EDITED', 'EDITED', 'APPROVED', 'DENIED');
