...
export class ALTERRecurrenceView1627927886724 implements MigrationInterface {
    public async up(queryRunner: QueryRunner): Promise<void> {
        // drop existing view and function
        await queryRunner.query('DROP VIEW IF EXISTS recurring_records_to_process');
        await queryRunner.query('DROP FUNCTION get_next_recurring_grant_scheduled_date;');
        // recreate function
        await queryRunner.query(`CREATE OR REPLACE FUNCTION get_next_recurring_grant_scheduled_date (start_date date, end_date date, frequency varchar, the_interval integer, num_grants integer)
            RETURNS date
            AS $$
        DECLARE
            today date;
        ...
        BEGIN
            ...
            END IF;
            SELECT
                INTO today the_current_date
            FROM
                recurring_grant_job_date;
        ...
        /** @note had to double escape all '\1' instances */
        await queryRunner.query(`
        CREATE OR REPLACE VIEW recurring_records_to_process AS
            SELECT
                view_data.*
            FROM (
                SELECT
                    ...
                    get_next_recurring_grant_scheduled_date (inner_view_data.start_date, inner_view_data.end_date, inner_view_data.frequency, inner_view_data.interval, inner_view_data.num_grants) AS next_scheduled_date,
                    ...
                    WHERE
                        ...
                        AND r.id NOT IN (
                            ...
                            FROM (
                                ...
                                SELECT
                                    inner_recurrence.id, inner_recurrence.start_date, inner_recurrence.end_date, inner_recurrence.frequency, inner_recurrence.interval, get_next_recurring_grant_scheduled_date (inner_recurrence.start_date, inner_recurrence.end_date, inner_recurrence.frequency, inner_recurrence.interval, num_grants) AS next_scheduled_date
                                FROM (
